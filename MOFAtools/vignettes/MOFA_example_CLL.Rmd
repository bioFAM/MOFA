...
* centerFeatures: logical indicating whether to center the features to zero mean. This is optional, the model can learn the feature-wise means by setting ModelOptions$learnIntercept to TRUE. This option only applies to continuous data that is modelled with a gaussian likelihood. Binary or count data are not centered.
* scaleViews: logical indicating whether to scale views to have unit variance. As long as the scale differences between the data sets is not too high, this is not required. Default is FALSE.
