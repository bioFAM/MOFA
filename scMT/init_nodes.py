...
class init_scGFA(initModel):
    ...
    def initAlpha(self, pa=1e-14, pb=1e-14, qa=1., qb=1., qE=1.):
    ...
    def initZeta(self):
        # Method to initialise the local variable zeta of the seeger approach
        Zeta_list = [None]*self.M
        for m in xrange(self.M):
            if self.lik[m] is not "gaussian":
                tmp = stats.norm.rvs(loc=0, scale=1, size=(self.N,self.D[m]))
                Zeta_list[m] = Zeta_Node(dim=(self.N,self.D[m]), initial_value=tmp) 
            else:
                Zeta_list[m] = None
        self.Zeta = Multiview_Local_Node(self.M, *Zeta_list)
