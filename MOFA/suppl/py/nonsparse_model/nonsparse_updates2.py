...
"""
Module to define the updates for the original non-sparse Group Factor Analysis model

Key differences with respect to MOFA:
- weights have a view-wise ARD sparsity prior but not an element-wise spike and slab sparsity prior. They are modelled using a multivariate gaussian
- latent variables are not fully factorised and modelled using a multivariate gaussian.
"""
...
class W_Node(MultivariateGaussian_Unobserved_Variational_Node):
    def __init__(self, dim, pmean, pcov, qmean, qcov, qE=None, qE2=None):
        MultivariateGaussian_Unobserved_Variational_Node.__init__(self, dim=dim, pmean=qmean, pcov=qcov, qmean=qmean, qcov=qcov, qE=qE)
        self.precompute()

    def precompute(self):
        self.D = self.dim[0]
        # self.K = self.dim[1] # IF WE ADD COVARAITES THIS SHOULD CHANGE
        self.factors_axis = 1
    ...
    def calculateELBO(self):
        ...
        lb_q = -self.D*self.dim[1] - logdet(Qpar['cov']).sum()
...
class Z_Node(MultivariateGaussian_Unobserved_Variational_Node):
    ...
    def precompute(self):
        self.N = self.dim[0]
        # self.K = self.dim[1] # IF WE ADD COVARIATES THIS SHOULD CHANGE
    ...
    def calculateELBO(self):
        ...
        lb_q = -self.N*logdet(Qpar['cov'][0,:,:]) - self.N*self.dim[1]
        return (lb_p - lb_q)/2
