...
#' @slot ImputedData the parsed data with the missing values imputed using the MOFA model. 
#'    A list with one matrix per view.
#' @slot Expectations expected values of the different variables of the model.
#'    A list of matrices, one per variable. The most relevant are "W" for weights and "Z" for factors.
#' @slot TrainStats list with training statistics such as evidence lower bound (ELBO), number of active factors, etc.
#' @slot DataOptions list with the data processing options such as whether to center or scale the data.
#' @slot TrainOptions list with the training options such as maximum number of iterations, tolerance for convergence, etc.
#' @slot ModelOptions list with the model options such as likelihoods, whether an intercept factors was learnt, etc.
#' @slot Dimensions list with the relevant dimensionalities of the model. N for the number of samples, 
