...
p1 <- showAllWeights(model, view="expr", factor="2", nfeatures=50, abs=FALSE, main="")
print(p1)
p2 <- showTopWeights(model, view="expr", factor="2", nfeatures=50,  manual_features=NULL)
print(p2)
```
```
...
anno_df <- data.frame(
  row.names=rownames(colData(model@InputData)),
...
showDataHeatmap(model, view="met_prom_cgi", factor="10", nfeatures=100, show_colnames=F, show_rownames=T, fontsize_row=8)
showDataHeatmap(model, view="Expression", factor="9", nfeatures=100, show_colnames=F, show_rownames=T, fontsize_row=8)
