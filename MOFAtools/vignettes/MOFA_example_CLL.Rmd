...
```{r}
plotWeightsHeatmap(MOFAobject, "Mutations", factors=c(1,2,3,4,5), show_colnames=F)
```
...
```{r}
FactorsScatterPlot(MOFAobject, factors = c("1", "2"), color_by = "IGHV", shape_by = "trisomy12")
```

An overview of pair-wise sctterplots for all or a subset of factors is produced by the 'FactorsScatterPairs' function
```{r}
FactorsScatterPairs(MOFAobject, factors = c("1", "2", "3", "4"), color_by = "IGHV")
```

A single factor can be visualised using the 'FactorBeeswarmPlot' function
```{r}
FactorBeeswarmPlot(MOFAobject, factors = "1", color_by = "IGHV")
```
...
FactorsScatterPlot(MOFAobject, factors=c("1","2"), color_by=h$cluster)
```
