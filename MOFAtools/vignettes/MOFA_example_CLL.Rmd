...
DirOptions <- list(
  ...
  "outFile" = 'CLL_model.hdf5' # Output file of the model (use hdf5 extension)
)
```
...
## Prepare MOFA
prepareMOFA internally performs a set of sanity checks, fills the TrainOptions and ModelOptions slots of the MOFAmodel object and it also creates a set of temporary files with the input matrices that will be loaded by the Python core implementation
```{r}
...
For example, here we plot all weights for factors 1 to 5 in the Mutation data:
```{r}
...
```{r}
plotWeights(MOFAobject, view = "Mutations", factor = 1, nfeatures = 5)
```
...
for (factor in interestingFactors) {
  lineplot <- LinePlot_FeatureSetEnrichmentAnalysis(MOFAobject, fsea.out, factor, threshold=0.01, max.pathways=10)
  print(lineplot)
}
```
