...
library(MultiAssayExperiment) # I think we shouldn't load this here
library(MOFAtools)
```
...
* Option 1: Bioconductor approach using the MultiAssayExperiment framework
* Option 2: base R approach using a list of matrices

## Option 1: bioconductor approach
...
# Load sample metadata (colData): Gender and Diagnosis
data("CLL_covariates")
head(CLL_covariates)
...
If using the base R approach, you simply need to provide a list of matrices where features are rows and samples are columns. The Importantly, the samples need to be aligned. Missing assays can be filled with NAs.
```{r}
data("CLL_data")
...
## Part 2: Characterisation of individual factors
### Inspection of top weighted features in the active views
To get an overview of the weights across all factors in a given view you can use the 'showWeightHeatmap' function.
For example, here we plot all weights from all factors in the Mutation data:
```{r}
showWeightHeatmap(MOFAobject, "Mutations", factors=c(1,2,3,4,5), show_colnames=F)
```
Similarly, we can also plot all weights for a single factor using the 'showAllWeights' function.
For example, here we plot all weights from Factor 1 in the Mutation data:
```{r}
showAllWeights(MOFAobject, view = "Mutations", factor = "1")
```
If you are only interested in looking at the top features you can use the 'showTopWeights' function.
...
```{r}
showTopWeights(MOFAobject, "mRNA", "1")
```

Finally, instead of looking at an the "abstract" weight, it is useful to observe the coordinated heterogeneity of the top features in the original data. This can be done using the 'showDataHeatmap' function.
```{r}
showDataHeatmap(MOFAobject, view="mRNA", factor="1", features=20, show_rownames=FALSE)
```
...
```{r}
scatterPairs(MOFAobject, factors = c("1", "2", "3", "4"), color_by = "IGHV")
```

A single factor can be visualised using the 'beeswarmPlot' function
```{r}
beeswarmPlot(MOFAobject, factors = "1", color_by = "IGHV")
```
...
Samples can be clustered according to their values on some or all latent factors using the 'clusterSamples' function. CLusters can for example be visualised using the scatterPlot function
```{r}
h <- clusterSamples(MOFAobject, k=2, factors="1")
scatterPlot(MOFAobject, factors=c("1","2"), color_by=h$cluster)
```
