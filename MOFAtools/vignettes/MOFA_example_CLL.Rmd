...
```{r}
FactorsScatterPlot(MOFAobject, factors = 1:2, color_by = "IGHV", shape_by = "trisomy12")
```

An overview of pair-wise sctterplots for all or a subset of factors is produced by the 'FactorsScatterPairs' function
```{r}
FactorsScatterPairs(MOFAobject, factors = 1:4, color_by = "IGHV")
```
...
FactorsScatterPlot(MOFAobject, factors=1:2, color_by=h$cluster)
```
