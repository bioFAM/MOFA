...
# Run MOFA
Once the MOFAobject is set up we can use runMOFA to train the model. As depending on the random initilization the results might differ, we recommend to use runMOFA multiple times (here ten times) and as a next step select the best model in the next step.
```{r}
n_inits <- 10
...
# Downstream analyses
On the trained MOFAobject we can now start looking into the inferred factros, its weights etc. 
```{r}
