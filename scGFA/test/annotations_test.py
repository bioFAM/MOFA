...
def run_test(use_annotations, seed=None, swap_factor=False):
    ...
    """
    data['Z'] = s.zeros((N,K))
    data['Z'][:,0] = stats.norm.rvs(loc=0, scale=1, size=N) # high variance one
    data['Z'][:,1] = stats.norm.rvs(loc=0, scale=1, size=N) # low variance one
    ...
    """
    # inferred data
    inferred_weights = pd.DataFrame()
    for i in range(M):
        SW_df = pd.DataFrame(SW.getExpectations()[i]['ESW'])
        tmp = pd.DataFrame(SW_df.stack())
        tmp = tmp.reset_index()
        tmp['view'] = i
        if inferred_weights.shape[0] ==0:
            inferred_weights = tmp
        else:
            inferred_weights = pd.concat([inferred_weights, tmp], 0)
    inferred_weights.columns = ['feature','factor', 'inferred', 'view']

    true_weights = pd.DataFrame()
    for i in range(M):
        data_W_df = pd.DataFrame(data['W'][i])
        tmp = pd.DataFrame(data_W_df.stack())
        tmp = tmp.reset_index()
        tmp['view'] = i
        if inferred_weights.shape[0] ==0:
            true_weights = tmp
        else:
            true_weights = pd.concat([true_weights, tmp], 0)
    true_weights.columns = ['feature', 'factor', 'true', 'view']

    all_weights = inferred_weights.merge(true_weights, on=['feature', 'factor', 'view'])
